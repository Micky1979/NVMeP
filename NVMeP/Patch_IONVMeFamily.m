                  ...
                  setProbeScore:(int)score
{
    ...
    UInt8  nvPlistPatchBytesR[] = {0x3C, 0x73, 0x74, 0x72, 0x69, 0x6E, 0x67, 0x3E, 0x70, 0x63, 0x69, 0x31, 0x34, 0x34, 0x64, 0x2C, 0x61, 0x38, 0x30, 0x32, 0x3C, 0x2F, 0x73, 0x74, 0x72, 0x69, 0x6E, 0x67, 0x3E};
    
    NSDictionary * nvPlistPatch = [NSDictionary dictionaryWithObjectsAndKeys:
                                   @"NVMe SSD IONameMatch", @"Comment",
                                   @"IONVMeFamily", @"Name",
                                   @"10.11.5,10.11.6", @"MatchOS",
                                   [NSNumber numberWithBool:YES], @"InfoPlistPatch",
                                   [NSData dataWithBytes:nvPlistPatchBytesF length:sizeof(nvPlistPatchBytesF)], @"Find",
                                   [NSData dataWithBytes:nvPlistPatchBytesR length:sizeof(nvPlistPatchBytesR)], @"Replace", nil];
    // -----------------------------------------------------
    ...
    if (![fm isWritableFileAtPath:[currentDir stringByAppendingString:@"/"]]) {
        printf("Error: %s is not writable!", currentDir.UTF8String);
        exit(1);
    }
    // kext to patch exist?
    ...
    // clean
    if ([fm fileExistsAtPath:newKext]) {
        if (![fm removeItemAtPath:newKext error:nil]) {
            printf("Error: can't remove old %s!\n", newKext.UTF8String);
            exit(1);
        }
    }
    
    if ([fm fileExistsAtPath:configPath]) {
        if (![fm removeItemAtPath:configPath error:nil]) {
    ...
    if ([info objectForKey:@"CFBundleName"]) {
        [info setObject:newKextExecutable forKey:@"CFBundleName"];
    }
    ...
    if ([info objectForKey:@"IOKitPersonalities"]
        ...
        if ([IOKitPersonalities objectForKey:@"GenericNVMeSSD"]
            ...
            if ([GenericNVMeSSD objectForKey:@"IONameMatch"]
                ...
                needPlistPatch = YES;
                NSMutableArray *IONameMatch = [NSMutableArray arrayWithArray:[GenericNVMeSSD objectForKey:@"IONameMatch"]];
                for (id pci in nvSupportedIds) {
                    if (![IONameMatch containsObject:pci]) {
                        [IONameMatch addObject:pci];
                    }
                }
            ...
            if ([GenericNVMeSSD objectForKey:@"IOPCIClassMatch"]) {
                ...
                if (AppleClassCode)
                {
                    printf("..leaving vanilla class code: ");
                }
                else
                {
                    printf("..checking class code:\n");
                    NSString *cc = [GenericNVMeSSD objectForKey:@"IOPCIClassMatch"];
                    if ([cc isEqualToString:nvPCClassCode]) {
                        printf("class code looks already as per standard, leaving as is: ");
                    }
                    else
                    {
                        printf("class code differ from PC standard (%s), is now set to: ", [cc UTF8String]);
                        [GenericNVMeSSD setObject:nvPCClassCode forKey:@"IOPCIClassMatch"];
                    }
                }
            }
            else
            {
                ...
                printf("..class code was missing, now is set to: ");
                [GenericNVMeSSD removeObjectForKey:@"IONameMatch"];
                [GenericNVMeSSD setObject:nvPCClassCode forKey:@"IOPCIClassMatch"];
            }

            printf("%s\n", [[GenericNVMeSSD objectForKey:@"IOPCIClassMatch"] UTF8String]);
#endif
            [IOKitPersonalities setObject:GenericNVMeSSD forKey:@"GenericNVMeSSD"];
        }
        
        [info setObject:IOKitPersonalities forKey:@"IOKitPersonalities"];
    }
    // adding credits
    ...
    [KernelAndKextPatches setObject:KextsToPatch forKey:@"KextsToPatch"];
    [configSample setObject:KernelAndKextPatches forKey:@"KernelAndKextPatches"];
    
    if ([configSample writeToFile:configPath atomically:YES]) {
        printf("Success creating %s!\n", configPath.UTF8String);
        printf("NOTE: for EC kexts may need to adjust the InfoPlistPatch to match your device id.\n");
        printf("      and off course if you install the kext, no need for the KextsToPatch!\n");
    }
    else
    {
